...
module.exports = function Mongoosastic(schema, options) {
  ...
  /**
   * Elasticsearch register percolator query function
   * @param id String - percolator id
   * @param query Object - query to percolate against
   * @param cb Function - cb when percolator is registered
   */
  schema.statics.percolator = function(id, query, cb) {
    setIndexNameIfUnset(this.modelName);
   ...
   */
  schema.statics.synchronize = function(query, bulkSize) {
    ...
    var stream = syncQuery.lean().cursor();
    ...
    stream.on('data', function(doc) {
      ...
      bulkCommand.push(serialize(doc, mapping));
      ...
      if (bulkCommand.length === bulkSize * 2) {
        ...
        esClient.bulk({body: bulkCommand}, function(err, result) {
          ...
          if (result.errors) {
            var errors = result.items.filter(x => x.index.error !== undefined);
            em.emit('error', errors);
          }
          bulkCommand = [];
          stream.resume();
        });
  ...
  /**
   * ElasticSearch percolate function
   *
   * @param cb - callback called with percolate results
   */
   schema.methods.percolate = function(cb) {
     setIndexNameIfUnset(this.constructor.modelName);
  ...
  function setIndexNameIfUnset(model) {
    ...
    if (namespace && !namespaceSet) {
      indexName = `${namespace}_${indexName}`;
      namespaceSet = true;
    }
  }
