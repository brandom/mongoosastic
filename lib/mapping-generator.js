...
function getTypeFromPaths(paths, field) {
  ...
  if (field === 'task_data' && paths[field].options.type.schemaName) {
    type = 'object';
  }
...
function getMapping(cleanTree, inPrefix) {
  ...
  for (field in cleanTree) {
    ...
    if (value.type === 'objectid') {
      ...
      // if ((value.ref) && value.flatten) {
      //   mapping[field].type = 'object';
      //   mapping[field].properties = {"_id": {type: "string"}};
      //   // TODO: FIX! This is making the assumption that all flattened paths are only strings
      //   value.flatten.split(' ').forEach((e) => mapping[field].properties[e] = {type: "string"});
      //   continue;
      // }
      // if (value.refPath && value.flatten) {
      //   mapping[field].type = 'object';
      //   continue;
      // }
...
function getCleanTree(tree, paths, inPrefix) {
  ...
  for (field in tree) {
    ...
    if (type) {
      ...
      // TODO: Make this better, but don't do above for now, assume virtual returns string
      if (typeof value === 'object' && value.getters && value.setters && value.options) {
        console.log('Got virtual', field)
        // cleanTree[field] = {
        //   type: 'string'
        // };
        // continue;
